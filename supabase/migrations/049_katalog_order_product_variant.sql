-- create_katalog_order: dukungan product_variant_id di item (F&B variant)
-- Item boleh berisi product_variant_id; harga dan nama pakai variant bila ada.
...
BEGIN
  ...
  LOOP
    ...
    WHERE product_id = v_product.id AND (unit_id = v_unit_id OR (v_unit_id IS NULL AND is_base))
    LIMIT 1;
    ...
    IF v_variant_id IS NOT NULL THEN
      IF NOT EXISTS (SELECT 1 FROM public.product_variants pv WHERE pv.id = v_variant_id AND pv.product_id = v_product.id AND pv.is_available) THEN
        v_variant_id := NULL;
      END IF;
    END IF;
    ...
    IF v_cust_price IS NOT NULL THEN
      v_price := v_cust_price;
    ELSIF v_variant_id IS NOT NULL THEN
      v_price := public.get_product_selling_price(v_product.id, v_variant_id);
      IF v_unit_id IS NOT NULL AND (SELECT is_base FROM public.product_units WHERE product_id = v_product.id AND unit_id = v_unit_id LIMIT 1) = false THEN
        v_price := v_price * v_conv;
      END IF;
  ...
  -- Insert order_items (dengan product_variant_id jika ada)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    SELECT * INTO v_product FROM public.products
    WHERE id = (v_item->>'product_id')::uuid AND organization_id = p_org_id LIMIT 1;
    ...
    v_variant_name := NULL;
    IF v_variant_id IS NOT NULL THEN
      SELECT name INTO v_variant_name FROM public.product_variants
      WHERE id = v_variant_id AND product_id = v_product.id AND is_available LIMIT 1;
      IF v_variant_name IS NULL THEN v_variant_id := NULL; END IF;
    END IF;
    ...
    ELSIF v_variant_id IS NOT NULL THEN
      v_price := public.get_product_selling_price(v_product.id, v_variant_id);
      IF (SELECT is_base FROM public.product_units WHERE product_id = v_product.id AND unit_id = v_unit_id LIMIT 1) = false THEN
        v_price := v_price * v_conv;
      END IF;
    ELSIF (SELECT is_base FROM public.product_units WHERE product_id = v_product.id AND unit_id = v_unit_id LIMIT 1) THEN
      v_price := v_product.selling_price;
    ...
    IF v_variant_id IS NOT NULL AND v_variant_name IS NOT NULL THEN
      v_display_name := v_product.name || ' - ' || v_variant_name || ' (' || v_sym || ')';
    ELSE
      v_display_name := v_product.name || ' (' || v_sym || ')';
    END IF;

    INSERT INTO public.order_items (order_id, menu_item_id, product_id, product_variant_id, unit_id, name, price, quantity, notes)
    VALUES (v_order_id, NULL, v_product.id, v_variant_id, v_unit_id, v_display_name, v_price, v_qty, NULL);
  END LOOP;
...
COMMENT ON FUNCTION public.create_katalog_order(uuid, jsonb, text, numeric) IS 'Buat pesanan katalog. Item: product_id, quantity, unit_id?, product_variant_id? (F&B).';
