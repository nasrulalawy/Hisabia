...
export function CatalogPage() {
  ...
  async function checkout() {
    ...
    try {
      ...
      setError(null);
      if (orgPhone && res?.orderToken) {
        const orderUrl = `${window.location.origin}/order/${res.orderToken}`;
        const waUrl = `https://wa.me/${orgPhone.replace(/\D/g, "").replace(/^0/, "62")}?text=${encodeURIComponent(
          `Halo, saya konfirmasi pesanan. Link detail: ${orderUrl}`
        )}`;
        ...
        {successOrder && (
          <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
            ...
            <p className="mt-1 text-xs">
              WhatsApp telah terbuka. Kirim ke toko untuk konfirmasi. Toko akan memproses pesanan dari link yang Anda kirim.
            </p>
          </div>
        )}
